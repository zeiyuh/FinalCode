...
#define BLYNK_AUTH_TOKEN "zjwKdIB_3ufC386UbkWMNebwAVfMf_xT"
...
bool normal = false; // bpm display
bool notnormal = false; // bpm display
...
void setup() {
      ...
      //  rtc.setTime(21, 30, 0);     // Set the dafault time HR.MIN.SEC
      //  rtc.setDate(22, 11, 2023);   // Set the default date DD.MM.YYYY
...
void loop() { 
        ...
        if (realHour == storedHour_1 && realMinute == storedMinute_1) { //alarm#1
          Blynk.logEvent("schedule_reminder");
          alarm = true; 
          on_slot1 == false; //ir 10 mins
          
          ir_1 =digitalRead(IRPin_1);
          if (ir_1 == LOW){ //not detected 
            StatusApp1();
            }
          else {   
            bool alarmloop1 = false;
            while (alarmloop1 == false){
               Blynk.run();
               ...
               advance = false;
               on_slot1 = true; // ir 10 mins
               dispense_1 = true; //  app supply
               IR_2();
        ...
        } else if (realHour == storedHour_2 && realMinute == storedMinute_2) { //alarm#2
          alarm = true;
          on_slot2 == false; //ir 10 mins
          ...
          if (ir_1 == LOW){ 
            ...
            bool alarmloop2 = false;
            while (alarmloop2 == false){
               Blynk.run();
               digitalWrite(irLED_1,LOW);
               soundAlarm();
               ...
               advance = false;
               on_slot2 = true; // ir 10 mins
               dispense_2 = true; //  app supply
               IR_2();
        ...
        } else if (realHour == storedHour_3 && realMinute == storedMinute_3) { //alarm#3
          alarm = true;
          on_slot3 == false; //ir 10 mins
          ...
          if (ir_1 == LOW){ 
            ...
            bool alarmloop3 = false;
            while (alarmloop3 == false){
               Blynk.run();
               digitalWrite(irLED_1,LOW);
               soundAlarm();
               ...
               advance = false;
               on_slot3 = true; // ir 10 mins
               dispense_3 = true; //  app supply
               IR_2();
        ...
        } else if (realHour == storedHour_4 && realMinute == storedMinute_4) { //alarm#4
          alarm = true;
          on_slot4 == false; //ir 10 mins
          ...
          if (ir_1 == LOW){ 
            ...
            bool alarmloop4 = false;
            while (alarmloop4 == false){
               Blynk.run();
               digitalWrite(irLED_1,LOW);
               soundAlarm();
               ...
               advance = false;
               on_slot4 = true; // ir 10 mins
               dispense_4 = true; //  app supply
               IR_2();
...
void zone_1(){
   while (y<1){ // duplicate if applicable
      String getInputFromKeyBoard(int n);
        lcd1.clear();
        lcd1.setCursor(0, 0);
        lcd1.print("*Set Hour:");
...
void Supplyapp_notif1(){
  if (S_appstore1 <= 3){
    Blynk.logEvent("slot1_supply_status","The medicines are running low in Slot#1. Please refill as soon as possible. Stay healthy!");
    }}
void Supplyapp_notif2(){    
  if (S_appstore2 <= 3){
    Blynk.logEvent("slot2_supply_status","The medicines are running low in Slot#2. Please refill as soon as possible. Stay healthy!");
    }}
void Supplyapp_notif3(){    
 if (S_appstore3 <= 3){
  Blynk.logEvent("slot3_supply_status","The medicines are running low in Slot#3. Please refill as soon as possible. Stay healthy!");
    }}
void Supplyapp_notif4(){       
  if (S_appstore4 <= 3){
  Blynk.logEvent("slot4_supply_status","The medicines are running low in Slot#4. Please refill as soon as possible. Stay healthy!");
  }}

void emptyslots(){
    if (S_appstore1 == 0){
     Blynk.logEvent("slot1_supply_status","Slot#1 is empty. Please refill as soon as possible. Stay healthy!");
    }
    if (S_appstore1 == 0){
     Blynk.logEvent("slot2_supply_status","Slot#2 is empty. Please refill as soon as possible. Stay healthy!");
    }
    if (S_appstore1 == 0){
     Blynk.logEvent("slot3_supply_status","Slot#3 is empty. Please refill as soon as possible. Stay healthy!");
    }
    if (S_appstore1 == 0){
     Blynk.logEvent("slot4_supply_status","Slot#4 is empty. Please refill as soon as possible. Stay healthy!");
    }}       
...
void BPMDISP(){ 
                uint32_t period_2 = 1 * 60000L;       // 15 loop
                for( uint32_t tStart_2 = millis();  (millis()-tStart_2) < period_2;  ){
                   ...
                   timedisplay();
                     ...
                     uint32_t pulseperiod_2 = 0.25 * 60000L;       // 15 loop
                     for( uint32_t pulseStart_2 = millis();  (millis()-pulseStart_2) < pulseperiod_2;  ){
                      ...
                      if (checkForBeat(irValue) == true){ // check for finger
                          ...
                          storage = beatAvg;
                          if (storage >=60 && storage <=100){
                            lcd2.clear();
                            lcd2.setCursor(0,0); 
                            lcd2.print (storage); 
                            lcd2.print(" is normal BPM");
                            delay(500);
                            bpmvalue_app1 = storage;
                            Blynk.virtualWrite(V20,bpmvalue_app1); 
                         ...
                         else if (storage !=0 && storage <=60 && storage >=100) {
                            lcd2.setCursor(0,0);  
                            lcd2.print (storage);
                            lcd2.print(" Abnormal BPM");
                            lcd2.setCursor(0,1);  
                            lcd2.print("Consult caregiver");
                            delay(500);
                            bpmvalue_app1 = storage;
                            Blynk.virtualWrite(V20,bpmvalue_app1);
...
void AppAdvance(){
      ...
      //  realTime.toInt() - storedMinute1.toInt() <=5


        
  while (advance == false){ 
  if (realMinute.toInt() - storedMinute_1.toInt() <=5 && realMinute. toInt() != storedMinute_1.toInt()){
    Blynk.logEvent("5_minutes_before_your_medication");
    advance = true;
    }else if (realMinute.toInt() - storedMinute_1.toInt() <=5 && realMinute. toInt() != storedMinute_2.toInt()){
      Blynk.logEvent("5_minutes_before_your_medication");
      advance = true;
      }else if (realMinute.toInt() - storedMinute_1.toInt() <=5 && realMinute. toInt() != storedMinute_3.toInt()){
        Blynk.logEvent("5_minutes_before_your_medication");
        advance = true;
        }else if (realMinute.toInt() - storedMinute_1.toInt() <=5 && realMinute. toInt() != storedMinute_3.toInt()){
          Blynk.logEvent("5_minutes_before_your_medication");
